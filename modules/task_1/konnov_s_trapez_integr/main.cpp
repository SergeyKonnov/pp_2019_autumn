...
TEST(Trapez_Integr, Test_Sequential_Integral) {
    ...
    if (rank == 0) {
        ...
        double i1 = GetTrapezIntegrSequential(2, 5, 1, f);
        double i2 = 39.5;
...
TEST(Trapez_Integr, Test_Sequetional_Integral_On_Sin_Function) {
    ...
    if (rank == 0) {
        ...
        double i1 = GetTrapezIntegrSequential(-10, 10, 0.5, f);
        double i2 = 0;
        EXPECT_NEAR(i1, i2, 1e-9);
    }
}
...
TEST(Trapez_Integr, Test_Parallel_Integral) {
    ...
    if (rank == 0) {
        double i2 = GetTrapezIntegrSequential(2, 5, 1, f);
        EXPECT_NEAR(i1, i2, 1e-9);
    }
}
...
TEST(Trapez_Integr, Test_Parallel_Integral_On_Sin_Function) {
    ...
    if (rank == 0) {
        double i2 = GetTrapezIntegrSequential(-10, 10, 0.5, f);
        EXPECT_NEAR(i1, i2, 1e-9);
    }
}
